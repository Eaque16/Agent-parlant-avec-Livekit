...
type RealtimeContextValue = { connect: (id: string) => Promise<void>; disconnect: () => void; session?: RealtimeSession; connected: boolean; room: Room }
...
export function LiveKitVoice({ children }: { children: React.ReactNode }) {
  ...
  const content = session ? <LiveKitRoom room={room} token={session.token} serverUrl={session.server_url} connect audio video={false}
