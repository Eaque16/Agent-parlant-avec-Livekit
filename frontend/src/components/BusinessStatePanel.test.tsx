...
describe('BusinessStatePanel',()=>{it('affiche la qualification structurée avec un libellé de confiance',()=>{render(<BusinessStatePanel state={state}/>);expect(screen.getByText('adhesion')).toBeInTheDocument();expect(screen.getByText('90 % fiable')).toBeInTheDocument();expect(screen.getByText('Étape 1 sur 3')).toBeInTheDocument();expect(screen.getByText('Identifier')).toBeInTheDocument()})})
