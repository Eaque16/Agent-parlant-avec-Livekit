...
class BusinessStateTracker:
    ...
    state_version: int = 0
    service: str = "inconnu"
    intent: dict[str, Any] = field(default_factory=lambda: {"label": "non_identifiee", "confidence": 0.0, "reformulation": "Besoin à préciser."})
    ...
    escalation: dict[str, Any] = field(default_factory=lambda: {"required": False, "type": "none", "reason": None, "queued_at": None, "ticket_id": None})
    ...
    def snapshot(self, increment: bool = True) -> dict:
        ...
        return {
            "conversation_id": self.conversation_id, "state_version": self.state_version,
            "updated_at": _now(), "language": "fr", "service": self.service,
            "intent": self.intent, "procedure": self.procedure, "step": self.step,
            "next_question": self.next_question, "collected": list(self.collected), "missing": list(self.missing),
            "resolution": self.resolution, "escalation": self.escalation,
            "risk_flags": list(dict.fromkeys(self.risk_flags)), "final_summary": self.final_summary, "is_final": self.is_final,
        }
...
def create_business_tools(tracker: BusinessStateTracker, publisher, procedures: list[dict]):
    ...
    async def set_business_state(service: str, intent_label: str, confidence: float, reformulation: str, next_question: str = "") -> str:
        tracker.service = service
        tracker.intent = {"label": intent_label, "confidence": max(0.0, min(1.0, confidence)), "reformulation": reformulation}
        tracker.next_question = next_question or None
        result = await publish()
        if result["backend"].get("ok"):
            rejected = result["backend"]["state"].get("rejected_fields", [])
            return "État publié et validé." + (f" Champs sensibles écartés ({', '.join(rejected)}) : ne les redemande pas." if rejected else "")
    ...
    async def lookup_procedure(service: str, intention: str) -> str:
        matches = [p for p in procedures if p.get("service_code") == service and intention.lower() in (p.get("name", "") + " " + p.get("id", "")).lower()]
        if not matches:
        ...
        tracker.step = {"index": 1, "total": len(selected.get("steps", [])), "label": selected.get("steps", [None])[0] if selected.get("steps") else None}
    ...
    async def collect_field(field: str, value: str) -> str:
        if sensitive_kinds(value):
            ...
            await publish()
            return "Information sensible refusée et non conservée. Ne la redemande pas ; explique qu'elle n'est pas nécessaire."
    ...
    async def request_escalation(type: str, reason: str, summary: str) -> str:
        tracker.escalation = {"required": True, "type": type, "reason": reason, "queued_at": None, "ticket_id": tracker.escalation.get("ticket_id")}
    ...
    return [set_business_state, lookup_procedure, collect_field, propose_resolution, request_escalation, refuse_action, finalize_call]
