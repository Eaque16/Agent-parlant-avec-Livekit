"""Validation, filtrage et persistance versionnée des états métier publiés par le worker."""
    ...
    with database.connection() as db:
        ...
        if current_row and payload["state_version"] <= current_row["state_version"]:
            _audit(
                ...
                "state_ignored",
                {"current_version": current_row["state_version"]},
            )
        ...
        for field in clean["collected"]:
            ...
            if kinds:
                ...
                _audit(
                    ...
                    clean["state_version"],
                    "pii_rejected",
                    {"field": field["field"], "kinds": sorted(kinds)},
                )
            else:
                kept.append(field)
        ...
        for path, container, key in (
            ...
            if kinds:
                ...
                _audit(
                    db, conversation_id, clean["state_version"], "pii_redacted", {"field": path, "kinds": sorted(kinds)}
                )
        ...
        if clean["escalation"]["required"]:
            ticket = get_or_create_ticket(
                db, conversation_id, clean["escalation"]["type"], clean["escalation"]["reason"]
            )
        ...
        db.execute(
            ...
            "ON CONFLICT(conversation_id) DO UPDATE SET state_version=excluded.state_version, "
...
def get_business_state(conversation_id: str) -> BusinessStateOut | None:
    with database.connection() as db:
        row = db.execute(
            "SELECT state_json FROM business_states WHERE conversation_id=?", (conversation_id,)
        ).fetchone()
...
def get_business_state_history(conversation_id: str) -> list[BusinessStateOut]:
    with database.connection() as db:
        rows = db.execute(
            "SELECT state_json FROM business_state_history WHERE conversation_id=? ORDER BY state_version",
            (conversation_id,),
        ).fetchall()
